...
%{-- Template pour gérer l'import du référentiel "Palier 3" depuis EmaEval dans Eliot --}%
...
<form id="form-importeReferentiel" style="display: none;">
  <p></p>
  <g:hiddenField id="emaEvalReferentielId" name="emaEvalReferentielId" value="" />
  <g:actionSubmit value="Importer le référentiel depuis EmaEval dans Eliot" action="importeReferentiel"/>
</form>

<r:script>
  var url = '<g:createLink action="verifiePreconditionImportReferentielEmaEvalVersEliot"/>';
  $.get(url, function(data) {
    ...
    if(data.emaEvalReferentielId) {
      $('#statut-EmaEvalReferentiel').html('<g:renderStatut value="${true}"/>');
      $('#emaEvalReferentielId').val(data.emaEvalReferentielId);
      $('#form-importeReferentiel').show();
    }
    else {
