...
class SujetServiceIntegrationTests extends GroovyTestCase {
  ...
  void testRendSujetCollaboratif() {
    given: "Un sujet non collaboratif avec du contenu"
    ...
    when: "On rend le sujet collaboratif"
    Sujet sujetCollaboratif = sujetService.rendSujetCollaboratif(
        personne1,
        sujetInitial,
        [personne2] as Set
    )
    ...
    assertEquals(
        [personne2]*.nomAffichage as Set,
        sujetCollaboratif.contributeurs*.nomAffichage as Set
    )
    sujetCollaboratif.questions.each {
      ...
      assertEquals(
          [personne2]*.nomAffichage as Set,
          it.contributeurs*.nomAffichage as Set
      )
    }
    assertNotSame(
        question1.id,
        sujetCollaboratif.questions[0].id
    )
    assertEquals(
        question1.titre,
        sujetCollaboratif.questions[0].titre
    )
    assertNotSame(
        question2.id,
        sujetCollaboratif.questions[1].id
    )
    assertEquals(
        question2.titre,
        sujetCollaboratif.questions[1].titre
    )

    given: "Un exercice comprenant 2 questions"
    ...
    and: "Un sujet comprenant l'exercice"
    Sujet sujetAvecExercice = sujetService.createSujet(personne1, 'sujetAvecExercice')
    sujetService.insertQuestionInSujet(
    ...
    when: 'On rend le sujetAvecExercice collaboratif'
    Sujet sujetCollaboratifAvecExercice = sujetService.rendSujetCollaboratif(
        personne1,
        sujetAvecExercice,
        [personne2] as Set
    )
    assertTrue(sujetCollaboratifAvecExercice.estCollaboratif())
    assertTrue(sujetCollaboratifAvecExercice.questions[0].estCollaboratif())
    Sujet exerciceCollaboratif = sujetCollaboratifAvecExercice.questions[0].exercice
    assertTrue(exerciceCollaboratif.questionComposite.estCollaboratif())
    assertEquals(
        exerciceCollaboratif,
        exerciceCollaboratif.questionComposite.sujetLie
    )
    exerciceCollaboratif.questions.each {
      assertTrue(it.estCollaboratif())
    }
    assertEquals(2, exerciceCollaboratif.questions.size())
  }
