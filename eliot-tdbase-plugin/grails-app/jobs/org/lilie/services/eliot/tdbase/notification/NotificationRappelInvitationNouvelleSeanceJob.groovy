 ...
 * @author John Tranier
 */
class NotificationRappelInvitationNouvelleSeanceJob {
    ...
    def getTriggers() {
        return config.eliot.tdbase.notifications.seance.rappelInvitation.trigger
    }
    ...
    private postNotification(Notification notification, ModaliteActivite seance) {
        ...
        try {
            ...
            if (rep.success == false) {
                ...
                seance.dateRappelNotificationOuvertureSeance = new Date()
                seance.save()
            }
        } catch (Exception e) {
            log.error(e.message)
